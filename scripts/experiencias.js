...
const pageConfig = PAGE_COLORS.experiencias;
const geometryConfig = GEOMETRY_CONFIG.experiencias;
const animConfig = ANIMATION_CONFIG.coreRotation.experiencias;

// Criar campo de estrelas
const starField = createStarField(18000, pageConfig.accentRgba);
scene.add(starField);

// Criar core (dodecahedron para experiências)
const core = createCore(geometryConfig.type, pageConfig.accent, 1.5);
scene.add(core);

// Criar partículas (apenas para experiências)
const particles = createParticleRing(pageConfig.accent, 200);
scene.add(particles);

// Configurar animações de scroll
setupScrollTimeline(camera, core, core.material, particles);
setupScrollAnimations();
setupCard3DTilt();
...
function animate() {
    ...
    particles.rotation.y += ANIMATION_CONFIG.particleRotation;
